...
class StandardDataset:
    ...
    def analyze(self, df_old, y=None, log=True, print_detail=False):
        df = df_old.copy()
        if y is not None:
        ...
        if self.name == "german":
            ...
            for lambd in [0, 10]:
                res[f"YNN-k-{k}-l-{lambd}"] = self.computeYNN(x, n, k, lambd, print_detail, df["y hat"].values)
